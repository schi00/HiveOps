...
using Serilog;
using SaaSBot.Api.HealthChecks;
using SaaSBot.Api.Hubs;
using SaaSBot.Api.Middleware;
using SaaSBot.Api.Services;
using SaaSBot.Api.Validation;
using SaaSBot.Agents;
using SaaSBot.Application.Interfaces;
using SaaSBot.Domain.Models;
using SaaSBot.Infrastructure;
...
builder.Host.UseSerilog((context, configuration) =>
{
    configuration
        ...
        .Enrich.WithProperty("ApplicationName", "SaaSBot")
        .ReadFrom.Configuration(context.Configuration);
});

// ── Infrastructure services ──────────────────────────────────────────────────
builder.Services.AddSaaSBotInfrastructure(builder.Configuration);
builder.Services.AddSaaSBotAgents();
...
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)
    .AddCookie(options =>
    {
        options.Cookie.Name = "saasbot.auth";
...
{
    c.SwaggerDoc("v1", new() { Title = "SaaSBot API", Version = "v1" });
