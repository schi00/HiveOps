...
public sealed class AdminTenantsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetBotMetrics([FromQuery] Guid? tenantId, CancellationToken ct)
    {
        ...
        var active = await conversations.CountAsync(c => c.Status == SaaSBot.Domain.Enums.ConversationStatus.Active, ct);
        var awaitingHuman = await conversations.CountAsync(c => c.Status == SaaSBot.Domain.Enums.ConversationStatus.AwaitingHuman, ct);

        var fallbackCount = await messages.CountAsync(m =>
            m.Role == SaaSBot.Domain.Enums.MessageRole.Assistant
        ...
        foreach (var conv in groupedByConv)
        {
            ...
            {
                if (item.Role == SaaSBot.Domain.Enums.MessageRole.User)
                {
                    lastUserMessage = item.CreatedAt;
                    continue;
                }

                if (item.Role == SaaSBot.Domain.Enums.MessageRole.Assistant && lastUserMessage.HasValue)
                {
        ...
        var fallbackPerDayRaw = await messages
            .Where(m => m.CreatedAt >= last30Days && m.Role == SaaSBot.Domain.Enums.MessageRole.Assistant)
